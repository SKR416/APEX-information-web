...
<body bgcolor="#FFF4C1">
	...
	<div class="box">
		<P Align="center">
			<font color="white" size="8" face="Microsoft JhengHei Light"><b>武器類別總覽</b></font>
		</P>
		<div class="wi">
			<?php
			include("conn.php");
			$SQL = "SELECT t.tname,w.wname FROM type t LEFT join weapon w on t.tid =w.tid order by t.tname ";
			$result = mysqli_query($link, $SQL);
			echo "<table border='1'>";
			echo "<tr><th>類別名稱</th><th>武器名稱</th></tr>";
			$prevType = $prevtext = $array = null;
			$count = 0;
			while ($row = mysqli_fetch_assoc($result)) {
				if ($prevType == null) {
					$prevType = $row["tname"];
					$prevtext = $row["wname"];
				} else if ($prevType == $row["tname"]) {
					$prevType = $row["tname"];
					$prevtext = $prevtext . ',' . $row["wname"];
				} else {
					$array[$count] = array($prevType,  $prevtext);
					$prevType = $row["tname"];
					$prevtext = $row["wname"];
					$count += 1;
				}
			...
			foreach ($array as $key => $value) {
				echo "<tr>";
				echo "<th>" . $array[$key][0] . "</th><th>" . $array[$key][1] . "</th>";
				echo "</tr>";
			}
